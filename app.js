...
app.get("/recipes/:recipeId", (request, response) => {
  Recipe.findById()
    .then(
      request.params.id.then((recipe) => {
        response.status(200).json(recipe);
      })
    )
    .catch((err) => {
      response.status(500).json({ message: "Error getting a recipe" });
    });
});
...
app.put("/recipes/:recipeId", (request, response) => {
  Recipe.findByIdAndUpdate(request.params.id, request.body, { new: true })
    .then((updateRecipe) => {
      response.status(200).json(updateRecipe);
    })

    .catch((err) => {
      response.status(500).json({ message: "Error updating a recipe" });
    });
});
...
app.put("/recipes/:recipeId", (request, response) => {
  Recipe.findByIdAndDelete(request.params.id)
    ...
    .catch((err) => {
      response.status(500).json({ message: "Error deleting a recipe" });
    });
});
